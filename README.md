...
- Админ-панель: поиск заказов, рассылка (текст/фото), управление статусами, обработка оплат, бан пользователей, изменение коэффициента для расчёта суммы клиенту.
- Реферальные начисления 15 % тому, кто подключил бота, при создании заказов через его токен.
- Автозапуск всех сохранённых ботов: основной токен и подключённые партнёры стартуют параллельно в своих папках `bd/<токен>/` с отдельными SQLite-базами.
- Рассылка из админки летит как в основной бот, так и во все подключённые бот-токены и их пользователей.
- Админ может посмотреть список всех владельцев подключённых ботов и статистику по каждому (пользователи, заказы, комиссия 15 %).
...
- Каталоги `db/` и `screens/` создаются автоматически рядом с `main.py` и работают одинаково в Windows и Linux.
...
- `DB_PATH`, `SCREENSHOTS_DIR`, `DB_DIR` — пути к базе и хранилищам файлов (по умолчанию относительные, от корня проекта). 
...
Проект распространяется по лицензии Apache 2.0. Полный текст — в файле [`LICENSE`](LICENSE). При использовании указывайте лицензионное уведомление и ссылку на лицензию.
