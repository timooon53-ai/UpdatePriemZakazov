...
Телеграм- и ВК-боты для приёма заявок на поездки/доставку, автоматического сбора данных от пользователя и пересылки заказов администраторам. Поддерживается подключение собственных ботов партнёрами: заказы пересылаются в основной бот, а подключившему партнёру начисляется комиссия.
...
- Админ-панель: поиск заказов, рассылка, управление статусами, обработка оплат, бан пользователей.
...
- ВК-бот с повторением ключевых сценариев: старт, помощь, оформление заказа, отмена.
- Логирование в файлы (`bot.log`, `vk_bot.log`) и консоль с поддержкой Unicode-эмодзи.

## Структура

- `main.py` — Telegram-бот на `python-telegram-bot` с пользователями, заказами, оплатами и админкой.
- `vk_bot.py` — ВК-бот на `vk_api` с базовыми этапами заказа.
...
- `VK_TOKEN`, `VK_GROUP_ID` — параметры ВК-бота.
